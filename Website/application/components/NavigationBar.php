    ...
    <div class="collapse navbar-collapse" id="navbar">
		...
		<ul class="navbar-nav ml-auto nav-flex-icons">
            <?php
                ...
                $statement = $sql->prepare("SELECT `money`, `next_reward` FROM `users` WHERE `id` = ?");
                $statement->execute([$_SESSION["user"]["id"]]);
                $result = $statement->fetch(PDO::FETCH_ASSOC);
                
                $money = $result["money"];
                $next_reward = intval($result["next_reward"]) - time();
                $human_readable = seconds2human($next_reward);
                $human_readable = $human_readable["days"] . " days, " . $human_readable["minutes"] . " minutes, and " . $human_readable["seconds"] . " seconds.";

                close_database_connection($sql, $statement);
            ?>

            <li class="nav-item mr-1" data-toggle="tooltip" data-placement="top" title="Next reward in <?= $human_readable ?>">
